<template>
    <div class="relative min-h-screen bg-black text-white overflow-hidden flex items-center justify-center px-4 py-12">
      ...
      <div class="relative z-10 bg-white rounded-2xl shadow-2xl flex flex-col lg:flex-row overflow-hidden w-full max-w-5xl animate-fade-in">
        ...
        <form @submit.prevent="editarInstalacion" class="w-full lg:w-1/2 p-8 space-y-5 text-black">
          ...
          <select v-model="instalacion.Estatus" class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500" required>
          ...
          <transition name="fade">
            <div v-if="exito" class="flex items-center justify-center gap-3 text-green-600 mt-4 animate-bounce">
              ...
              <span>Instalación editada exitosamente</span>
            </div>
          </transition>
        </form>
  ...
  export default {
    ...
    async mounted() {
      const instalacionId = this.$route.params.id;
      await this.obtenerInstalacion(instalacionId);
    },
    methods: {
      async obtenerInstalacion(id) {
        try {
      ...
      async editarInstalacion() {
        try {
          ...
          // Enviar la solicitud PUT a la API para editar
          const instalacionId = this.$route.params.id;
          const response = await api.put(`/instalacion/${instalacionId}`, instalacionActualizada);
