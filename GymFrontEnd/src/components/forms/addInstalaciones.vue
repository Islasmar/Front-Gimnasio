    ...
    <div class="relative min-h-screen bg-black text-white overflow-hidden flex items-center justify-center px-4 py-12">
      ...
      <div class="relative z-10 bg-white rounded-2xl shadow-2xl flex flex-col lg:flex-row overflow-hidden w-full max-w-5xl animate-fade-in">
        ...
        <form @submit.prevent="agregarInstalacion" class="w-full lg:w-1/2 p-8 space-y-5 text-black">
          ...
          <input v-model="instalacion.sucursal" type="text" placeholder="Sucursal"
          ...
          <input v-model="instalacion.estatus" type="text" placeholder="Estatus"
            class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500" required />
          
          <input v-model="instalacion.fechaRegistro" type="date"
            class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500" required />
    ...
    methods: {
      async agregarInstalacion() {
        try {
          ...
          const nuevaInstalacion = { ...this.instalacion, fecha_registro: fecha };
  
          // Simulación de guardado
          setTimeout(() => {
            console.log('Instalación guardada (simulada):', nuevaInstalacion);
            this.cargando = false;
            this.exito = true;
